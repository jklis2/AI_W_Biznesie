...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const systemMessage = generatePcSetSystemMessage();
    const userPrompt = `User query: "${message}"
Available components and preferences:
${JSON.stringify(preferences, null, 2)}

Please provide PC build recommendations based on these preferences.`;
...
function highlightKeyFeatures(responseText: string): string {
  const keyFeatures = responseText.match(/- \*\*[^:]+:\*\* [^\n]+/g) || [];
  return keyFeatures.join('\n');
}

function generatePcSetSystemMessage(context?: string): string {
  ...
  if (context === 'gaming') {
    systemMessage = `You are an expert in building gaming PCs. Prioritize components that enhance gaming performance, such as a powerful GPU and fast RAM. Explain the advantages of the proposed configuration for gaming.`;
  } else if (context === 'streaming') {
    systemMessage = `You are an expert in building streaming PCs. Prioritize components that enhance streaming quality, such as a multi-core CPU and fast RAM. Explain the advantages of the proposed configuration for streaming.`;
  } else if (context === 'programming') {
    systemMessage = `You are an expert in building programming workstations. Prioritize components that enhance development workflow, such as a fast CPU, ample RAM, and fast storage. Explain the advantages of the proposed configuration for programming.`;
  }

  systemMessage += ` Format response in Markdown with clear sections and bullet points.`;
  return systemMessage;
}
...
// OpenAI-like wrapper for Sonar API
const openai = {
  chat: {
    completions: {
